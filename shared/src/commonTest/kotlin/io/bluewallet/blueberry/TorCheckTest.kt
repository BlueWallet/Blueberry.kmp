...
class TorCheckTest {
    @Test
    fun checkTorExit_ok_shows_elapsed_not_ip() =
        runBlocking {
            var now = 10_000L
            val lines =
                checkTorExit(
    ...
    @Test
    fun checkTorExit_retries_IsTor_false_then_succeeds() =
        runBlocking {
            var now = 0L
            var calls = 0
            val lines =
                checkTorExit(
                    ...
                    fetchOnce = {
                        ...
                        now += 1_000
                        if (calls == 1) {
                            TorCheckIp(isTor = false, ip = "9.9.9.9")
                        } else {
                            TorCheckIp(isTor = true, ip = "8.8.8.8")
                        }
                    },
                )
    ...
    @Test
    fun checkTorExit_reports_error_when_deadline_elapses() =
        runBlocking {
            val lines =
                checkTorExit(
                    overallMs = 0,
