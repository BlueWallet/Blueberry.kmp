...
private fun upsertAlive(db: io.bluewallet.blueberry.storage.Database, host: String) {
    db.peers.upsert(
        PeerWrite(
            host = host,
            port = 8333,
...
private fun connectAccepting(
    received: MutableList<Transaction>,
): suspend (String, Int, Job) -> ByteDuplex = { _, _, _ ->
...
class BroadcastModuleTest {
    @Test
    fun waits_for_alive_peer_delivers_tx_and_reports_ok() = runBlocking {
    ...
    @Test
    fun retries_other_alive_peers_after_dial_failure() = runBlocking {
        val db = createSqliteDatabase(":memory:")
    ...
    @Test
    fun dial_timeout_retries_the_next_peer() = runBlocking {
        val db = createSqliteDatabase(":memory:")
        upsertAlive(db, "1.1.1.1")
        upsertAlive(db, "2.2.2.2")
        val bus = createMessageBus()
    ...
    @Test
    fun reports_failure_after_exhausting_attempts() = runBlocking {
        val db = createSqliteDatabase(":memory:")
        upsertAlive(db, "1.1.1.1")
        val bus = createMessageBus()
        val done = async {
    ...
    @Test
    fun cancel_while_waiting_for_alive_peers() = runBlocking {
        val db = createSqliteDatabase(":memory:")
        val bus = createMessageBus()
        var dialed = 0
        val done = async {
    ...
    @Test
    fun second_request_is_rejected_while_in_progress() = runBlocking {
        val db = createSqliteDatabase(":memory:")
        val bus = createMessageBus()
