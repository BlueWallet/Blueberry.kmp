...
class V2BroadcastTest {
    ...
    @Test
    fun succeeds_on_ack_timeout_when_peer_stays_silent() =
        runBlocking {
            val (client, server) = pairedByteDuplexes()
    ...
    @Test
    fun succeeds_when_peer_closes_after_tx() =
        runBlocking {
            val (client, server) = pairedByteDuplexes()
            val peer =
                async {
    ...
    @Test
    fun fails_when_peer_sends_reject() =
        runBlocking {
            val (client, server) = pairedByteDuplexes()
            val peer =
                async {
    ...
    @Test
    fun rejects_invalid_hex_before_handshake() =
        runBlocking {
            val (client, server) = pairedByteDuplexes()
            val error =
    ...
    @Test
    fun handshake_timeout_when_peer_never_speaks() =
        runBlocking {
            val (client, server) = pairedByteDuplexes()
            val error =
                assertFailsWith<IllegalStateException> {
