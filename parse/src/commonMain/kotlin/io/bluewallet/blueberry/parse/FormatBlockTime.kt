 ...
 */
fun formatRelativeAge(unixSeconds: Long?, nowMs: Long): String {
...
private fun nextAgeBoundary(ageS: Long, bucket: Long): Long =
    if (ageS >= 0) ((ageS / bucket) + 1) * bucket
    else -(((-ageS) / bucket) * bucket - 1)

/** Milliseconds until [formatRelativeAge] would change, or null if the label is static. */
fun msUntilNextRelativeAgeChange(unixSeconds: Long?, nowMs: Long): Long? {
