...
class PeerProbeTest {
    ...
    @Test
    fun wantAddr_collects_addrv2_after_handshake_and_skips_onion() = runBlocking {
        coroutineScope {
            val (clientSide, serverSide) = pairedByteDuplexes()
            val server = async {
    ...
    @Test
    fun wantAddr_caps_a_stream_of_one_address_messages() = runBlocking {
        coroutineScope {
            val (clientSide, serverSide) = pairedByteDuplexes()
            val server = async {
                serveHandshake(serverSide) { protocol ->
    ...
    @Test
    fun wantAddr_timeout_after_handshake_still_returns_ok_with_empty_peers() = runBlocking {
        coroutineScope {
            val (clientSide, serverSide) = pairedByteDuplexes()
            val server = async {
                serveHandshake(serverSide) { protocol ->
    ...
    @Test
    fun wantAddr_timeout_also_bounds_a_blocked_pong_write() = runBlocking {
        coroutineScope {
            val (clientSide, serverSide) = pairedByteDuplexes()
            var blockWrites = false
            val client = object : ByteDuplex {
    ...
    @Test
    fun parent_cancel_during_addr_wait_is_not_a_failed_handshake() = runBlocking {
        coroutineScope {
            val (clientSide, serverSide) = pairedByteDuplexes()
