...
private fun internalHashHex(payload: BlockPayload): String = bytesToHex(sha256d(encodeBlockHeader(payload.header)))
...
): suspend (String, Int, BlockSyncOptions) -> BlockBatchResult<BlockSessionApi> =
    { host, _, _ ->
        onOpen?.invoke(host)
        val session =
            object : BlockSessionApi {
...
class BlocksDownloadTest {
    @Test
    fun start_emits_progress_from_db_downloads_persists_marks_peer_used() =
        runBlocking {
    ...
    @Test
    fun successful_peer_is_never_reused_for_another_block() =
        runBlocking {
            val bus = createMessageBus()
            val db = createSqliteDatabase(":memory:")
    ...
    @Test
    fun instant_successes_do_not_take_the_stalled_peer_wait_path() =
        runBlocking {
            val bus = createMessageBus()
            val db = createSqliteDatabase(":memory:")
    ...
    @Test
    fun does_not_reuse_a_used_peer_when_it_is_the_only_NODE_NETWORK_peer() =
        runBlocking {
            val bus = createMessageBus()
            val db = createSqliteDatabase(":memory:")
            val b0 = makeVariantBlock(0u)
    ...
    @Test
    fun idle_resumes_on_filters_match() =
        runBlocking {
            val bus = createMessageBus()
            val db = createSqliteDatabase(":memory:")
            val block = makeVariantBlock(0u)
    ...
    @Test
    fun idle_picks_up_a_new_match_even_if_the_kick_is_lost() =
        runBlocking {
            val bus = createMessageBus()
            val db = createSqliteDatabase(":memory:")
            val b0 = makeVariantBlock(0u)
    ...
    @Test
    fun in_flight_download_does_not_block_a_later_match() =
        runBlocking {
            val bus = createMessageBus()
            val db = createSqliteDatabase(":memory:")
            val b0 = makeVariantBlock(0u)
    ...
    @Test
    fun failed_downloads_retry_under_concurrency_until_complete() =
        runBlocking {
            val bus = createMessageBus()
            val db = createSqliteDatabase(":memory:")
    ...
    @Test
    fun keeps_polling_while_pending_remain_no_bus_kick_required() =
        runBlocking {
            val bus = createMessageBus()
            val db = createSqliteDatabase(":memory:")
            val block = makeVariantBlock(0u)
    ...
    @Test
    fun discards_in_flight_block_after_reorg_replaces_the_match_hash() =
        runBlocking {
            val bus = createMessageBus()
            val db = createSqliteDatabase(":memory:")
    ...
    @Test
    fun hash_mismatch_nothing_persisted_peer_not_marked_used() =
        runBlocking {
            val bus = createMessageBus()
            val db = createSqliteDatabase(":memory:")
    ...
    @Test
    fun openSession_throw_does_not_kill_the_download_loop() =
        runBlocking {
            val bus = createMessageBus()
            val db = createSqliteDatabase(":memory:")
            val block = makeVariantBlock(0u)
    ...
    @Test
    fun honors_idleDelayMs_when_a_match_appears_without_a_kick() =
        runBlocking {
            val bus = createMessageBus()
            val db = createSqliteDatabase(":memory:")
            val block = makeVariantBlock(0u)
    ...
    @Test
    fun while_sync_idle_peers_updated_does_not_open_sessions() =
        runBlocking {
            val bus = createMessageBus()
            val db = createSqliteDatabase(":memory:")
            val b0 = makeVariantBlock(0u)
    ...
    @Test
    fun stop_cancels_an_in_flight_block_request_before_joining() =
        runBlocking {
            val bus = createMessageBus()
            val db = createSqliteDatabase(":memory:")
